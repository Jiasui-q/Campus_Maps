...
public class StreetMapGraph implements AStarGraph<Long> {
    ...
    private Map<Point, Long> findId = new HashMap<>();
    ...
    public StreetMapGraph(String filename) {
        ...
        for (Node node: nodes.values()) {
            if (!neighbors.get(node.id()).isEmpty()) {
                Point p = new Point(projectToX(node.lon(), node.lat()), projectToY(node.lon(), node.lat()));
                list.add(p);
                findId.put(p, node.id);
            }
        }
        for (Node node: nodes.values()) {
            if (node.name() != null) {
                termsList.add(new Term(node.name(), node.importance()));
            }
        }
    }
    ...
    public long closest(double lat, double lon) {
        ...
        KDTreePointSet kdTree = new KDTreePointSet(list);
        Point result = kdTree.nearest(x, y);
        return findId.get(result);
    }
    ...
    public List<String> getLocationsByPrefix(String prefix) {
        ...
        BinaryRangeSearch sort = new BinaryRangeSearch(terms);
        Term[] finalTerms = sort.allMatches(prefix);
        for (Term fTerm: finalTerms) {
            result.add(fTerm.query());
        }
        return result;
    }
